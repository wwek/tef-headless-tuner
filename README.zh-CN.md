...
- **I2C 上拉电阻**：SDA 和 SCL 各 4.7kΩ 上拉至 3V3

无需外接 LDO — ESP32-S3 DevKit 板载 3V3 输出可直接给 TEF6686 模块供电。如果使用成品 TEF6686 模块（如 PE5PVB 开发板），晶振和周围阻容已在模块上。
...
TCP 端口 7373，兼容 XDR-GTK 桌面调谐器软件。支持 SHA1 挑战认证和标准 XDR 协议命令（T、M、Y、D、A、W、C、x）。状态推送频率约 15 Hz。
