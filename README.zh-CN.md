...
当前仓库默认配置面向 `16MB` Flash 的 `ESP32-S3` 开发板
（例如 `N16R8` 或同等级别模组）。如果你使用的是更小 Flash 的板子，
需要先调整 `sdkconfig` 和 `partitions.csv`，再进行烧录。
...
- 支持原生 USB Device 且默认配置下具有 `16MB` Flash 的 `ESP32-S3` 开发板
